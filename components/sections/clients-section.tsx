...
export const ClientsSection = ({clientLogos}: ClientsSectionProps) => {
  ...
  return (
    <Section backgroundHeight="full" className="pt-8 md:pt-12 xl:pt-16 2xl:pt-24">
      {clientLogos.map(({logo, alt}, index) => (
        <FadeIn key={index} direction="up" className="h-20" delay={breakpoint.lg ? index * 0.1 : breakpoint.md && index % 2 != 0 ? 0.1 : 0}>
          <div className="relative mx-auto h-full w-full max-w-[200px] opacity-70">
            <Image src={logo} alt={alt} placeholder="blur" blurDataURL={logo} layout="fill" objectFit="contain" objectPosition="center" />
          </div>
        </FadeIn>
      ))}
    </Section>
